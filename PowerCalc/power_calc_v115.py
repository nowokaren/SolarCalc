...
file_run = "../Runs/" + name + "_"+ title
# report = pot.start_report(file, name)
dic_paneles = pot.read_resumen(file_run+"/"+name+"-resumen.txt", ["Panel "])
len_paneles = len(dic_paneles.keys())
dic_cases = pot.read_resumen(file_run+"/"+name+"-resumen.txt", ["- Dirección óptima -  caso periodoWC", "- Dirección óptima -  caso simulacion2", "- Dirección óptima -  caso simulación1",])
...
calc_cons, T, porcentaje_celdas, consumo_file, case, caras, cells, distrib_type, sup_disp, power_case, report, distribucion= pot.set_params_condiciones(mision, file_run, n_cases, len_paneles)
...
fechas_gen, duracion_gen, dist_sol, cos, eclip_gen, head, datos_gen = pot.read_cos(file_run, report, case = case)
datos = pot.read_resumen(file_run+f"/{name}-resumen.txt", ["Periodo", "Duración del eclipse más largo"])
file = file_run+ "/Power_Calc/"+power_case+"/" 
