...
def set_params_condiciones(cond, report):
    ...
    if calc_cons:
        print("---- Calculo el consumo posible según la distribución de celdas por caras definida por la usuaria.")
        report.write("---- Calculo del consumo posible según la distribución de celdas por caras definida por la usuaria.")
        report.write("\n")
       ...
       report.write("---- Calculo del la distribución de celdas por caras según el perfil de consumo definido por la usuaria.")
    T		=	cond.getfloat("T")          # °C
...
def set_title(Epoch, SMA, ECC, INC, RAAN, AOP, TA, ARB, BAV0, BAV1, BAV2, ACT, BCV0, BCV1, BCV2, Attitude = "NadirPointing"):
    for i  in [BAV0, BAV1, BAV2, BCV0, BCV1, BCV2,]:
       if i<1 and i>0:
           i=str(i)[2:]
    title = f"Ep{Epoch[3:6]}{Epoch[12:14]}-SMA{np.round(SMA)}-ECC{str(ECC)[2:]}-INC{np.round(INC,1)}-RAAN{np.round(RAAN)}-AOP{np.round(AOP)}-TA{TA}_{Attitude[:3]}{ARB[0]}{BAV0}{BAV1}{BAV2}-{ACT[0]}{BCV0}{BCV1}{BCV2}"
    return title
...
def pot_total(fechas, P_faces,  fechas_cons, P_cons, cell_faces, area_cell, periodo, report, file, graph = None, case = ""):
    tiempo = to_tiempo(fechas)
...
def distribution_cells(consumo_total, gen_faces, head, area_cell, report, caras = 1, distribucion = 1, case = ""):
    '''Si len(caras) != 1: hay que elegir valor para distribución distinto de 1'''
    gen_tot = []
    posiciones = orden(head, gen_faces, report)
...
def balance_energy(n, fechas_cons, E_cons, fechas_gen, E_tot, report, file, graph = None, case = ""):
    ...
    if tiempo_neg_balance != []:
        ...
        print(f"Energía mínima necesaria en la batería: {-np.round(batery_cap/1000,2)} kJ = {-np.round(batery_cap/3600000,8)} kWh\n            (Energía acumulada para valores negativos del balance)")
        report.write("\n")
...
def calc_distribution_cell(E_gen_tot, input_cells, head, cells, area_cell, report, sup_disp = 1, posiciones =1):
    ...
    if "Max" in input_cells: 
        ...
        if len(cells) != len(head):
            cells = ast.literal_eval(input(f"cell_faces la dimensión de cell_faces no coincide con la cantidad de paneles con cosenos calculados ({len(head)}). Ingrese una lista de dimensión {len(head)} de la cantidad de celdas por cara:\n"))
        sup_cells = cells
        cell_faces = [np.floor(i/(area_cell/10000)) for i in cells]
        for i_cara in range(len(cells)):
            sup_cell = sup_cells[i_cara]
            gen_panel = cell_faces[i_cara] * E_gen_tot[i_cara]
            if cell_faces[i_cara] > 0:
        ...
        print("input_cells INVALIDO")
        input_cells = input("Ingrese un valor válido para input_cells:\n")
        cell_faces, sup_cells = calc_distribution_cell(input_cells, head, cells, area_cell, sup_disp, posiciones)
    return cell_faces, sup_cells