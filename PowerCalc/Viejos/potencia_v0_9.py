...
def set_params_condiciones(cond, report):
    ...
    return T, porcentaje_celdas, consumo_file, case, caras
...
def perfil_consumo(file_name, hoja,  start_date, report, graph = None):
...
def pot_faces(fechas, cos, Px, head, graph = None, case = ""): 
...
def pot_total(fechas, P_faces,  fechas_cons, P_cons, cell_faces, area_cell, report, graph = None, case = ""):
    tiempo = to_tiempo(fechas)
...
def energy_faces(fechas, P_faces, head, area_cell, report, graph = None, case = ""):
    E_faces = []
    E_faces_tot = []
    tiempo = to_tiempo(fechas)
...
def energy_total(fechas, E_faces, cell_faces, report, graph = None, case = ""):
...
def to_periodic(n, fechas0, potencia0, start_date,  tiempo_max, report, graph = None):
    ...
    print(f"El proceso de agrandar periodicamente el consumo duró: {t1-t0}")
    report.write("\n")
    report.write(f"El proceso de agrandar periodicamente el consumo duró: {t1-t0}")
    if graph != None:
        plt.figure(graph)
    ...
    if len(caras) > 1:
        ...
        if caras == "Max":
            for j, i in enumerate(distribucion):
                ...
                print('{:^10} {:^5} {:^15} {:^15} {:^15}'.format(f'{head[i_cara][-7:]}', f'{i*100}%', f'{np.round(gen_panel/3600000,2)}', f'{int(np.ceil(cell_per_panel))}', f'{np.round(sup_cells[i_cara],2)}'))
                report.write("\n")
                report.write('{:^10} {:^5} {:^15} {:^15} {:^15}'.format(f'{head[i_cara][-7:]}', f'{i*100}%', f'{np.round(gen_panel/3600000,2)}', f'{int(np.ceil(cell_per_panel))}', f'{np.round(sup_cells[i_cara],2)}'))
...
def balance_energy(n, fechas_cons, E_cons, fechas_gen, E_tot, report, graph = None, case = ""):
    ...
    report.write("\n")
    report.write(f"Energía total extra que necesito: {-np.round(batery_cap/1000,2)} kJ\t{-np.round(batery_cap/3600000,8)} kWh (Batería)")
...
def balance(n, Px, cos, head, fechas_gen, fechas_cons, E_cons, porcentaje_celdas, area_cell, graph_P_tot):
    P_faces = pot_faces(fechas_gen, cos, Px, head)
    E_faces, E_faces_tot = energy_faces(fechas_gen, P_faces, head)
    
    posiciones = orden(head, E_faces_tot)
    
    
    gen_tot, cell_faces, sup_cells = distribution_cells(porcentaje_celdas, E_cons[-1], E_faces_tot, head, posiciones, area_cell)
    
    P_tot = pot_total(fechas_gen, P_faces, cell_faces, graph = graph_P_tot)
    E_tot = energy_total(fechas_gen, E_faces, cell_faces, graph = "Energía")
    
    tiempo_bis, balance = balance_energy(n, fechas_cons, E_cons, fechas_gen, E_tot, graph = "Energía")
    return P_faces, E_faces, E_faces_tot, P_tot, E_tot, tiempo_bis, balance
