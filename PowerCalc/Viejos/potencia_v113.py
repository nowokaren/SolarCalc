...
def set_params_condiciones(cond, file, n):
...
def pot_faces(fechas, cos, Px, head, area_cell, file, Potencia, graph = None, case = ""): 
    ...
    Potencia[case[:3],"Tiempo", "datetime"] = fechas
    for i, cara in enumerate(head):
        Potencia[case[:3], cara[3:], "W"] = P_faces[:,i]
    if graph != None:
...
def pot_total(fechas, P_faces,  fechas_cons, P_cons, cell_faces, area_cell, periodo, report, file, Potencia, graph = None, case = ""):
    ...
    Potencia[case[:3], "Total", "W"] = P_total
...
def energy(fechas, potencia, report = None, graph = None, case = None,):
    ...
    if case != None:
        ...
        Consumo["Tiempo", "datetime"] = tiempo
        Consumo["Potencia", "W"] = potencia
        Consumo["Energía", "J"] = E
        Consumo["Energía", "kWh"] = np.array(E)/3600000
        if type(fechas[0]) == datetime.datetime:
            Consumo["Tiempo", "s"] = tiempo
            Consumo["Tiempo", "datetime"] = fechas
        else: 
            Consumo["Tiempo", "s"] = fechas
...
def energy_faces(fechas, P_faces, head, area_cell, report, file, Energia, graph = None, case = ""):
    ...
    print("\n")
    report.write("\n")
    Energia[case[:3],"Tiempo", "datetime"] = fechas
    Energia[case[:3],"Tiempo", "s"] = tiempo
    for i, cara in enumerate(head):
        Energia[case[:3], cara[3:], "J"] = E_faces[i]
        Energia[case[:3], cara[3:], "kWh"] = E_faces[i]/3600000
...
def energy_total(fechas, E_faces, cell_faces, report, file, Energia, graph = None, case = ""):
    ...
    Energia[case[:3], "Total", "J"] = E_tot
    Energia[case[:3], "Total", "kWh"] = np.array(E_tot)/3600000
...
def balance_energy(n, fechas_cons, E_cons, fechas_gen, E_tot, report, file, Energia, graph = None, case = ""):
    ...
    Energia[case[:3], "Balance", "kJ"] = balance/1000
    Energia[case[:3], "Balance", "kWh"] = balance/3600000
    Energia[case[:3], "Balance_time", "s"] = tiempo_bis
    Energia[case[:3], "Balance_time", "datetime"] = fechas_bis
