...
def set_params_condiciones(cond, file):
    ...
    if calc_cons:
        power_case = "C-"
        if distrib_type == "NxF":
            power_case += distrib_type+f"-{cells}celdas"
        elif distrib_type == "SxF":
            power_case += distrib_type+f"-{cells}m2"
        elif "Max" in distrib_type:
            power_case += distrib_type+f"-SupDisp{sup_disp}-Cells{cells}"
    else:
        power_case = "G-"
        if distrib_type == "%":
            power_case += "porcentaje"
            distribucion = porcentaje_celdas
        elif distrib_type == "sup_disp":
            power_case += "sup_disp"
...
def calc_distribution_cell(E_gen_tot, distrib_type, head, cells, area_cell, report, sup_disp = 1):
