...
"""
import datetime
import gmat_set_orbit_v1_1 as gso
import orbital_mec_v1_1 as om
import numpy as np
import configparser
import os
import shutil
...
name, Epoch, SMA, ECC, INC, RAAN, AOP, TA, ARB, BAV, ACT, BCV, step, duration_run, script_name, paneles = gso.set_params_mec_orbit(settings["mecanica orbital"])
...
if duration_run > corte_subrun:
    ...
    # Reporte de settings
    gso.print_report(file, name, values, period, num_orbWC[:,1][-1], paneles, duration_run2, step, max(duration), case = "WC")
    ...
    gso.print_report(file, name, values, period, num_orbWC[:,1][-1], paneles, duration_run, step,  max(duration))
