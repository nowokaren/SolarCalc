...
def data_orbit(file, paneles, scale = 1000, case = 1):
    ...
    - paneles: (list) Lista de los vectores normales de los paneles desplegables en el sistema body.
...
def subrunWC_params(start, stop, duration, t_orb, period):
...
def plano_max(file, cos1, case = 1, graph = True):
    ...
    print(f"---- La normal al plano que maximiza el coseno es [{np.round(x,3)}, {np.round(y,3)}, {np.round(z,3)}] para el caso {case}.")
    if graph:
