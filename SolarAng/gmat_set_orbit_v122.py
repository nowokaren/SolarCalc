...
def print_report(file, name, values, period, num_orbs, axes, duration_run, step, name_plano_max, duracion_eclip_max = 1, case = 1):
    ...
    if case == 1:
        ...
        if len(axes) != 6:
            for j,i in enumerate(axes[6:-1]):
                if len(i)>1:
                    report.write(f"\n\tPanel {j+1}:\t{np.round(list(i[1]),4)}")
                else:
                    report.write(f"\n\tPanel {j+1}:\t{np.round(i,4)}")
            if name_plano_max != []:
                report.write(f"\n- Dirección óptima -  caso simulación 1 o larga =\t{np.round(axes[-1][1],4)}\t= Panel\t{len(axes[6:])}")
        else:
            report.write(" None")
        ...
        for i in range(len(name_plano_max)):            
            report.write(f"- Dirección óptima -  caso {name_plano_max[len(name_plano_max)-1-i]} =\t{np.round(axes[len(axes)-1-i][1],4)}\t= Panel\t{len(axes)-6-i}\n")
    report.close()
...
def time_interval(file, name, values, num_orbWC, step, start, end, axes, paneles, period = 1, name_plano_max = 1, eclip_dur = 1, case_in = 1, case_out = "periodoWC" ) :      
    if f'Sol-NormalSat-{case_out}.png' in os.listdir(file):
        try:
            m = max([j for j in os.listdir(file) if "Sol-NormalSat-" in j])
            i = int(m[-5:-4])+1
            case_out += str(i)
        except ValueError:
            i = 1
            case_out+=str(i)
    t_orb, cos, dist_sun = om.select_timedelta(file, start, end, case = case_in)
    cos, axes, paneles, n_max, Z_max, plano_max_s, name_plano_max = om.add_plano_max(file, cos, axes, paneles, name_plano_max, case = case_out)
    om.report_cos(file, t_orb, dist_sun, cos, case = case_out)
    om.report_ang(file, t_orb, dist_sun, cos, case = case_out)
    cos = om.add_shadow(cos)
    om.graph_cos(t_orb, cos, file, dist_sun, case = case_out, line = True, period = period, eclip_dur = eclip_dur)
    duration_run = round((end - start).total_seconds()/(24*60*60),5)
    print_report(file, name, values, period, num_orbWC[:,1][-1], axes, duration_run, step, name_plano_max, eclip_dur, case = case_out)
    return t_orb, cos, dist_sun, axes, paneles, n_max, Z_max, plano_max_s, name_plano_max