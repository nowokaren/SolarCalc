...
def graph_cos(t_orb, cos, file, dist_sun, case = 1, line = False, period = 1, eclip_dur = 1):
    ...
    if case != 1:
        title+="-"+case
    if case == "periodoWC" and period!=1:
        ax[1].set_title(f"Periodo = {str(period)} s")
    if case == "periodoWC" and period!=1 and eclip_dur != 1:
        ax[1].set_title(f"Periodo = {str(period)} s    Eclipse = {str(eclip_dur)} s")
    if case == "periodoWC" and eclip_dur!=1:
        ax[1].set_title(f"Eclipse = {str(eclip_dur)} s")
...
def add_plano_max(file, cos, axes, paneles, name_plano_max, graph = True, case = 1):
    ...
    axes.append(np.array([[0,0,0],n_max]))
