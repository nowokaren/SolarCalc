...
if duration_run > corte_subrun:
    ...
    indexWC, cosWC, dist_sunWC, ax1, axesWC= om.cosenos(euler_angsWC, positionWC, t_orbWC, X_orbWC, Y_orbWC, Z_orbWC, paneles, file, scale_sol, scale_actitud, scale_panel, calc_sun = False, graph = True, sun_vect=sun_vectWC)
    ...
    if duration != []:
        ...
        eclipI, eclip = om.getpos_eclip(t_orbWC, positionWC, start, stop, duration, num_orbWC[:,1], file, ax1, axesWC, graph = True, case = "WC")  # Calcula las posiciones del satétlite en las que está eclipsado
        cosWC = om.add_eclip(cosWC, eclipI, eclip_stamp = np.nan)
...
if duration != []:
    ...
    if duration_run > corte_subrun:
        ...
        gso.print_report(file, name, values, period, num_orbWC[:,1][-1], axes3, duration_run, stepWC, name_plano_max, max(duration), case = "periodoWC")

# Si se desea tomar un "pedazo" de los datos de cosenos del resultado de la simulacion 1 (case_in = 1) o 2 (case_in="WC"), además del caso del peor período, se puede usar la siguiente función en la consola:

startWC -= dt.timedelta(minutes = 30)
endWC -= dt.timedelta(minutes = 30)
t_orb4, cos4, dist_sun4, axes4, paneles4, n_max4, Z_max4, plano_max_s4, name_plano_max4 = gso.time_interval(file, name, values, num_orbWC, stepWC, startWC, endWC, axes3, paneles3, period = period, name_plano_max = name_plano_max3, eclip_dur = max(duration), case_in = "WC", case_out = "pruebita" )