...
def set_params_condiciones(cond, file, n_cases, len_panels):
    ...
    if calc_cons:
        ...
        if distrib_type == "%":
            ...
            while len(sup_disp)!= n+6:
                sup_disp = list(map(float,ast.literal_eval(input(f"La longitud de la variable sup_disp no coincide con la cantidad de planos de la simulación. Definir nuevamente sup_disp con longitud {n+6}:\n"))))
            sup = "".join([str(round(s)) for s in sup_disp])
    ...
    if "Power_Calc" not in os.listdir(file):
        ...
        report = open(file+"/Power_Calc/"+power_case+"/"+power_case+"-resumen.txt", "w")
    report.write(f"---------- {case} Power_Case = {power_case} ----------")
    report.write("\n")
    if calc_cons:
...
def read_cos(file, report, case = 1):
    ...
    try:
        ...
        path_ = file+"/angulos/"+name_cos+".txt"
        print(f"--!-- El archivo que contiene los cosenos con path \"{path_}\" no existe. (Si pertenece a una corrida de 2 simulaciones, la corrida vacia \"\" no existe.)")
        raise SystemExit
...
def read_resumen(file, datos):
    try:
        ...
        print("El archivo \"-resumen.txt\" no fue generado o los parámetros que se encuentra en settings.ini no coinciden con una corrida hecha previamente con SolarAng.")
        raise SystemExit
    dic = {}
    j=0
    for i in f:
...
def pot_faces(fechas, cos, Px, head, area_cell, file, Potencia, graph = None, case = ""): 
    ...
    for i, cara in enumerate(head):
        Potencia[f"{case[:3]}-{cara[3:]}-W"] = [round(p,3) for p in P_faces[:,i]]
    if graph != None:
...
def pot_total(fechas, P_faces,  fechas_cons, P_cons, cell_faces, area_cell, periodo, report, file, Potencia, graph = None, case = ""):
    ...
    Potencia[f"{case[:3]}-Total-W"] = [round(p,3) for p in P_total]
...
def energy_faces(fechas, P_faces, head, area_cell, report, file, Energia, graph = None, case = ""):
    ...
    for i, cara in enumerate(head):
        Energia[f"{case[:3]}-{cara[3:]}-J"] = [round(e,3) for e in E_faces[i]]
        Energia[f"{case[:3]}-{cara[3:]}-kWh"] = [round(e,3) for e in E_faces[i]/3600000]
...
def energy_total(fechas, E_faces, cell_faces, report, file, Energia, graph = None, case = ""):
    ...
    Energia[f"{case[:3]}-Total-J"] = [round(e,3) for e in E_tot]
    Energia[f"{case[:3]}-Total-kWh"] = [round(e,3) for e in np.array(E_tot)/3600000]
...
def to_periodic(new, n, fechas0, potencia0, start_date,  tiempo_max, report, graph = None):
    ...
    try:
        ...
        print("--!-- El valor de n = duracion_gen/duracion_cons es nulo o infinito. El periodo de tiempo evaluado debe tener mayor duración.")
        raise SystemExit
    for i in range(1,n):
...
def balance_energy(n, fechas_cons, E_cons, fechas_gen, E_tot, report, file, Energia, graph = None, case = ""):
    ...
    Energia[f"{case[:3]}-Balance-kJ"] = [round(e,3) for e in balance/1000]
    Energia[f"{case[:3]}-Balance-kWh"] = [round(e,3) for e in balance/3600000]
...
def add_degT(Imp, Vmp, T, T_Imp, T_Vmp, T_ref = 28):
    Pmp = (Imp+T_Imp*(T-T_ref)) * (Vmp+T_Vmp * (T-T_ref))
    if Pmp <= 0.00001:
        return 0
    else:
        return Pmp
